  ...
  """
    ...
    if mode == "train":
        ...
        example_queue = tf.FIFOQueue(
            3 * batch_size, dtypes=[tf.float32, tf.int32], shapes=[[image_size, image_size, depth], [1]]
        )
        num_threads = 1
    ...
    labels = tf.sparse_to_dense(tf.concat(values=[indices, labels], axis=1), [batch_size, num_classes + 1], 1.0, 0.0)
    labels = tf.slice(labels, [0, 0], [batch_size, num_classes])
